...
//DELETING TASK COMMENT
	$(document).on("click", ".comments td .removeNote", function() {
			// hover starts code here
				...
				var taskID = $(this).attr("taskid");
				var dataString = {'type':"deleteComment",'commentID':commentID,'taskID':taskID};
...
//getting filtered todo list information 	
	function getFilteredTodoList(theID, filterType) {
			var dataString = {'type':"getFiltered",'filterType':filterType,'theID':theID,};
				$.ajax({
				type: "POST",
				url: "/dashboard/todolist/process.php",
				data: dataString,
...
//disable close top av
	...
	$(document).on('click','.todoCheck:not(.todoChecked)', function(e) {
		...
		var dataString = {'type':"checkTodoItem",'todoID':todoID,'value':"yes"};		
				$.ajax({
					type: "POST",
					url: "/dashboard/todolist/process.php",
					data: dataString,
	...
	$(document).on('click','.todoChecked', function(e) {
		...
		var dataString = {'type':"unCheckTodoItem",'todoID':todoID,'value':"no"};		
				$.ajax({
					type: "POST",
					url: "/dashboard/todolist/process.php",
					data: dataString,
	...
	$(document).on('click','.editTodo', function(e) {
		...
		if ($(this).find("i").hasClass("fa-floppy-o")) {
			...
			var dataString = {'type':"updateTodoItem",'todoID':todoID,'title':selectedVal,'priorityID':priorityID};		
				$.ajax({
					type: "POST",
					url: "/dashboard/todolist/process.php",
					data: dataString,
	...
	$(document).on('click','.deleteTodo', function(e) {
		...
		var dataString = {'type':"deleteTodoItem",'todoID':todoID};		
				$.ajax({
					type: "POST",
					url: "/dashboard/todolist/process.php",
					data: dataString,
	...
	$('#addNewTodoItem').on('click', function(e) {
		...
		var dataString = {'type':"addTodoItem",'title':value,'priorityID':priorityID};
				$.ajax({
					type: "POST",
					url: "/dashboard/todolist/process.php",
					data: dataString,
...
if ($(window).width() < 728) {
   ...
   $(this).find("td:eq(2)").remove();
	...
	$(document).on('click','.no_click', function(){return false;});
...
"use strict";	
	...
	$( "#notificationSort .formLabels" ).click(function(e) {
			...
			$(".printNotifications:visible").hide("slide", { direction: "right" }, 500);
			$.when($(".printNotifications:visible").hide("slide", { direction: "right" }, 300)).then(function(){ $('#'+controls).show("slide", { direction: "right" }, 500); });
	...
	$( "#clearNotifications,#clearNotificationsMobile" ).click(function(e) {
		$(".notificationCount").hide();
  		var whichNotif = $("#notificationSort .formLabels.active").attr("whichNotif");
				...
				type: "POST",
				url: "/dashboard/notifications/process.php",
				data: {'type':"clearAll",'whichNotif':whichNotif},
				cache: false,
				success: function(result){
					getAllNotif();
				}
	...
	$( "#readNotifications,#readNotificationsMobile" ).click(function(e) {
				...
				type: "POST",
				url: "/dashboard/notifications/process.php",
				data: {'type':"readAll",'whichNotif':whichNotif},
				cache: false,
				success: function(result){
				getAllNotif();
				}
...
//CHANGING NOTIFICATIONS TO SEEN //
	$(document).on('click','.printNotifications div.unseen', function(e) {
			...
			var notificationID = $(this).attr("id");	
				var dataString = {'type':"seen",notificationID1:notificationID};	
	...
	$(document).on('click','#alertContainer .fa-times', function() {
				...
				var dataString = {'type':"markAlert",alertID:alertID};	
