...
$(document).ready(function() {
 ...
 // Make it round!
  ...
  $(".ripple").css({
	...
	//appending menu		
	$("#projectsTableContainer").append('<div class="selectedMenu"><div class="heading"><h3>Actions</h3><div id="closeMenu"><i class="fa fa-times-circle-o" aria-hidden="true"></i></div></div><ul><li id="viewInfo">View <i class="fa fa-external-link" aria-hidden="true"></i></li><li id="archive">Archive</li><li id="reactivate">Reactivate</li><li id="changeStatus" class="hasSecondaryMenu">Change Status</li><li id="changeStatusSecondaryMenu" controller="changeStatus" class="secondaryMenu"><select><option value="Incomplete">Incomplete</option><option value="In Progress">In Progress</option><option value="Complete">Complete</option></select><button class="genericbtn">Save</button></li><li id="changeTitle" class="hasSecondaryMenu">Change Title</li><li id="changeTitleSecondaryMenu" controller="changeTitle" class="secondaryMenu"><input type="text"><button class="genericbtn">Save</button></li><li id="delete">Delete</li></ul></div>');
    ...
    left: xPos + '%'
		...
		$(document).on("click",".selectedMenu li",function() {
			...
			if (type === "archive" || type === "reactivate" || type === "delete"){
				
				$.alertable.confirm('Are you sure? This cannot be undone!').then(function() {
					//closing menu
					$( "#closeMenu").trigger( "click" );
					
					var dataString = {'type':type,'projectID':projectID};
						...
						success: function(result){
								...
								reloadSuccess(entries, projectID);
						},
				...
				var currentVal = selectedRow.find("td:nth-child(6) div").text();
		...
		$(document).on("click",".secondaryMenu button",function() {
			...
			if (type === "changeStatus") {
				$.alertable.confirm('Are you sure?').then(function() { 
					...
					var dataString = {'type':type,'projectID':projectID,'newVal':newVal};
		...
		$.alertable.confirm('Are you sure you want to archive?').then(function() {
		$( ".projectsTable tbody tr td input[type='checkbox']" ).each(function() {
			...
			{
				projectIDs = $.grep(projectIDs, function(value) {
					...
					$.ajax({
				    		url: 'process.php',
				    		data: 'type=archiveMultiple&projectIDs='+projectIDs,
				    		type: 'POST',
							success: function(result){
								var entries = result.printBack;
								reloadSuccess(entries, projectIDs);
						},
						error: function(result){
							alert("Error.");
						}
		...
		$.alertable.confirm('Are you sure you want to reactivate?').then(function() {
		$( ".projectsTable tbody tr td input[type='checkbox']" ).each(function() {
			var projectID = $(this).attr("projectid");
			  if ($(this).is(':checked')) {
			...
			else 
			{
				projectIDs = $.grep(projectIDs, function(value) {
					...
					$.ajax({
				    		url: 'process.php',
				    		data: 'type=reactivateMultiple&projectIDs='+projectIDs,
				    		type: 'POST',
							success: function(result){
								var entries = result.printBack;
								reloadSuccess(entries, projectIDs);
		...
		$( ".projectsTable tbody tr td input[type='checkbox']" ).each(function() {
			...
			else 
			{
				projectIDs = $.grep(projectIDs, function(value) {
					...
					$.ajax({
				    		url: 'process.php',
				    		data: 'type=deleteMultiple&projectIDs='+projectIDs,
				    		type: 'POST',
							success: function(result){
								var entries = result.printBack;
								reloadSuccess(entries, projectIDs);
      ...
      <div class="row">
		 ...
		 <div class="col-sm-12" id="moveOver">
			<div class="whitebg">
					...
					<div class="header">
					<h3><strong><a href="/dashboard/settings/">Settings:</a></strong> Project Management</h3>
			...
			<div class="col-sm-12">
				...
				<div id="projectsTableContainer" class="table-responsive projectsSettingsTable">
				<table class="projectsTable" id="printBack">
					<thead>
					<tr>
						<th>Title</th>
						<th>Category</th>
						<th id="orderBy">Date Created</th>
						<th>Due Date</th>
						<th>Owner</th>
						<th>Status</th>
						<th>Check All</th>
