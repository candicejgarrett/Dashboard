...
$ticketID = $_POST["ticketID"];
	...
	$getInfo = "SELECT `TicketID`, `Title`, `Description`, `URL`, DATE_FORMAT(`Tickets`.`Timestamp`, '%b %e, %Y @ %l:%i %p') AS 'Timestamp', DATE_FORMAT(`Tickets`.`Due Date`, '%b %e, %Y @ %l:%i %p') AS 'Due Date', `Contact Name`, `Contact Email`, `Owner`, `Status`, `ProjectID`, `Tickets Categories`.`Category` FROM `Tickets` JOIN `Tickets Categories` ON `Tickets Categories`.`CategoryID` = `Tickets`.`Category` WHERE `TicketID`= '$ticketID'";
	...
	while($row = mysqli_fetch_array($getInfo_result)) {
		$ticketRequestedBy = $row["Contact Name"]."<br><a href='mailto:".$row["Contact Email"]."'>".$row["Contact Email"]."</a>";
		$ticketTitle = $row["Title"];
		$ticketOwner = $row["Owner"];
		$ticketDescription = $row["Description"];
		$ticketDueDate = $row["Due Date"];
		$ticketCategory = $row["Category"];
		$ProjectID = $row["ProjectID"];
	}
	
		$headers = "From: info@candicejgarrett.com\r\n";
		$headers .= "Reply-To: candice.garrett@burlingtonstores.com\r\n";
		$headers .= "CC: candice.garrett@burlingtonstores.com\r\n";
		$headers .= "MIME-Version: 1.0\r\n";
		$headers .= "Content-Type: text/html; charset=ISO-8859-1\r\n";

if ($type == "newTicket") {
	//getting members
	$getProjectMember = "SELECT * FROM `user` WHERE `userID` = '$ticketOwner'";
	...
	$subject = "A new ticket has been submitted: ".$ticketTitle.".";
        ...
        <table cellspacing="0" cellpadding="0" width="600" class="w320">
          ...
          <tr>
            <td class="free-text">
             ...
             <div class="task"><p class="pull-left" style="line-height:23px;"> <strong>Launch Date: </strong><br>'.$ticketDueDate.'</p>
				 <p class="pull-right" style="line-height:23px;"> <strong>Category: </strong><br>'.$ticketCategory.'</p>
             <br><br><h2 style="clear:both">'.$ticketTitle.'<br><span style="font-weight:300; font-style: italic;font-size:14px;color:#4801ff;line-height:23px;">Submitted By: '.$ticketRequestedBy.'</span></h2>
             
             </div>
            
             <a href="https://candicejgarrett.com/dashboard/requests/view.php?ticketID='.$ticketID.'" class="button">View Ticket</a>
              <br><br>
            </td>
          </tr>
